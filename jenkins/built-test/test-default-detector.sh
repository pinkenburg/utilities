...
if (( $run_valgrind > 0 )); then
	...
	valgrind_prefix="valgrind -v --num-callers=30 --gen-suppressions=all --max-threads=2500 --leak-check=full --error-limit=no --log-file=${macro_name}.valgrind $valgrind_sup --xml=yes --xml-file=${macro_name}.valgrind.xml --leak-resolution=high"
