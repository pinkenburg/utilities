		...
		stage('build-cppcheck')
		{
			...
			{			
				writeFile file: "cppcheck.sh", text:'''#!/usr/bin/env bash
					...
					cppcheck -q --inline-suppr  --enable=warning --enable=performance --platform=unix64 --inconclusive --xml --xml-version=2 -j 10 --std=c++17 --suppress=unknownMacro --suppress=noCopyConstructor --suppress=noOperatorEq ./OnlMon  > cppcheck-result.xml 2>&1
					status=${PIPESTATUS[0]}
