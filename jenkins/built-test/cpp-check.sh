...
cppcheck -q --inline-suppr  --enable=warning --enable=performance --platform=unix64 --inconclusive --xml --xml-version=2 -j 10 --std=c++20 --check-level=exhaustive --suppress=syntaxError --suppress=unknownMacro --suppress=noCopyConstructor --suppress=noOperatorEq ./coresoftware > & cppcheck-result.xml
