	...
	stages { 
				...
				timestamps {
					ansiColor('xterm') {
						...
						script {
							...
							currentBuild.displayName = "${env.BUILD_NUMBER} - ${sha_macros}"
							currentBuild.description = "${upstream_build_description} / <a href=\"${git_url_macros}\">macros</a> # ${sha_macros}" 
								...
								[
						   		...
						   		extensions: [               
							     	...
							     	[
							    		...
							    		options: [
											mergeRemote: 'origin',
							  			mergeTarget: "$mergeTarget_macros"
							  			]
							    	],
						   		],
							  	branches: [
							  	...
							  	userRemoteConfigs: 
							  	[[
							    	//credentialsId: 'sPHENIX-bot', url: 'https://github.com/sPHENIX-Collaboration/macros.git'
	    ...
	    {	      
	      steps 
	      {
			...
			// recordIssues qualityGates: [[threshold: 0.5, type: 'NEW', unstable: true], [threshold: 0.5, type: 'NEW_HIGH', unstable: true], [threshold: 2000, type: 'NEW', unstable: false], [threshold: 50, type: 'NEW_HIGH', unstable: false]], tools: [clangTidy(pattern: 'clang-tidy-result.txt')]	      
